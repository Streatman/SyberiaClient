class GearPDAMenu extends UIScriptedMenu
{
	...
	ref Widget m_page_msg;
	ref Widget m_page_map;
	ref Widget m_page_grp;
	...
	{
		layoutRoot = GetGame().GetWorkspace().CreateWidgets( "IntenZ/Syberia/layouts/IZ_GearPDAMenu.layout" );
		...
		m_yourIdText = TextWidget.Cast( layoutRoot.FindAnyWidget( "your_id_label" ) );
		...
		m_mute_label = TextWidget.Cast( layoutRoot.FindAnyWidget( "mute_btn_label" ) );
		m_group_help_text = TextWidget.Cast( layoutRoot.FindAnyWidget("group_info") );
		
		m_addContactTxt = EditBoxWidget.Cast( layoutRoot.FindAnyWidget( "add_contact_box" ) );
		m_message = EditBoxWidget.Cast( layoutRoot.FindAnyWidget( "send_msg_box" ) );
		...
		m_page_msg = layoutRoot.FindAnyWidget( "PagePDA" );
		m_page_map = layoutRoot.FindAnyWidget( "PageMap" );
		m_page_grp = layoutRoot.FindAnyWidget( "PageGroups" );
	...
	void UpdateMap()
	{
		...
		m_map.AddUserMark(pos, "", ARGBF(1, 0.690, 0.152, 0.188), "IntenZ\SyberiaClient\pda\imagesets\marker.edds");
		m_map.SetScale(0.05);
	...
	void UpdateButtonsPagePanel()
	{
		...
		pagewgts.Insert(m_page_msg_btn);
		pagewgts.Insert(m_page_map_btn);
		pagewgts.Insert(m_page_grp_btn);
		
		m_page_msg_btn.GetSize(w, h);
	...
	void UpdateMutedButton()
	{
		...
		{
			m_mute_label.SetText("#pda_unmute");
		}
		else
		{
			m_mute_label.SetText("#pda_mute");
		}
	}
	...
	protected void ColorHighlight( Widget w )
	{
		...
		ButtonSetColor(w, ARGB(172, 32, 37, 18));
		ButtonSetTextColor(w, ARGB(255, 150, 169, 87));
	}
	...
	protected void ColorNormal( Widget w )
	{
		...
		ButtonSetColor(w, ARGB(76, 32, 37, 18));
		ButtonSetTextColor(w, ARGB(255, 50, 55, 29));
	}
