class SkillsMenu extends UIScriptedMenu
{
	...
	ref Widget m_perkHint_panel;
	ref Widget m_perkErr_panel;
	...
	{
		layoutRoot = GetGame().GetWorkspace().CreateWidgets( "IntenZ/Syberia/layouts/IZ_SkillsMenu.layout" );
		...
		m_perkHint_panel = layoutRoot.FindAnyWidget("PerkHint_panel");
		m_perkHint = MultilineTextWidget.Cast(layoutRoot.FindAnyWidget("PerkHint"));
		m_perkErr_panel = layoutRoot.FindAnyWidget("PerkErr_panel");
		...
		m_activatePerkBack = layoutRoot.FindAnyWidget("PerkSelect_panel");
	...
	private void UpdateSkillsList()
	{
		...
		{
			rowId = m_skillLevelsList.AddItem("#syb_skill_level1 " + perkUnlockLevel, new Param1<int>(perkUnlockLevel), 0);
			...
			else if (visualState == 1) visualState = ARGB(255, 0, 128, 32);
			else visualState = ARGB(255, 0, 0, 0);
	...
	private void UpdatePerksList()
	{
		...
		foreach (ref PerkInfo perk : perks)
		{
			...
			else if (visualState == 1)
			{
				...
				{
					m_skillPerksList.SetItemColor(rowId, 0, ARGB(255, 0, 128, 32));
				}
				else
				{
					m_skillPerksList.SetItemColor(rowId, 0, ARGB(255, 255, 0, 0));
	...
	{	
		...
		if (selectedSkillRow >= 0 && selectedPerk >= 0 && selectedLevel >= 0)
		{
			...
			if (selectedLevelValue && selectedLevelValue.param1 > 0)
			{
				...
				if (perk && player && player.m_skills)
				{
					...
					else if (perkStatus == -3) errorText = "#syb_perk_err2 (#syb_skill" + selectedSkill + " " + perk.GetPrevUnlockLevel(selectedLevelValue.param1) + " #syb_skill_level2)";
				}
			}
		}
		...
		m_activatePerkBtn.Show(perkStatus == 0);
		m_perkHint_panel.Show(showHint);
		m_perkErr_panel.Show(perkStatus < 0);
