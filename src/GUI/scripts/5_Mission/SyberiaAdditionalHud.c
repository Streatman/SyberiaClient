class SyberiaAdditionalHud
{
	...
	void CreateEspMarker(vector pos, string name, int type, int color)
	{
		...
		ref Widget w = GetGame().GetWorkspace().CreateWidgets("IntenZ/SyberiaClient/GUI/layout/AdminToolMarker.layout");
		...
		{
			iw.LoadImageFile(0, "IntenZ\SyberiaClient\configs\data\gui\Markers\corpse.paa");
		}
		else if (type == PluginAdminTool_EspType.VEHICLES)
		{
			iw.LoadImageFile(0, "IntenZ\SyberiaClient\configs\data\gui\Markers\car.paa");
