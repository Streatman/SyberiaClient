class ScreenNewchar extends ScreenBase
{
	...
	{
		layoutRoot = GetGame().GetWorkspace().CreateWidgets( "IntenZ/Syberia/layouts/IZ_ScreenNewchar.layout" );
	...
	private void UpdateSkillsPreview()
	{
		...
		m_currentSkillInc.Show( currentScore > 0 && selectedSkillValue < 100 );
		m_currentSkillDec.Show( selectedSkillValue > 0 );
	}
